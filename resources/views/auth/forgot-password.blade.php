...
<head>
    ...
    <title>Signal Recovery | ZENITH</title>
    <style>
...
<body class="auth-gradient text-slate-300 min-h-screen flex items-center justify-center p-4 sm:p-6 relative overflow-hidden scanline">
    ...
    <div class="absolute inset-0 opacity-20 pointer-events-none">
        ...
        <div class="absolute top-[-20%] right-[-10%] w-[60%] h-[60%] bg-cyan-500/10 rounded-full blur-[120px] animate-pulse"></div>
    </div>

    <div class="w-full max-w-[440px] relative z-10">
        
        <div class="text-center mb-8">
            <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-slate-900/50 border border-white/10 mb-6 shadow-2xl relative group">
            ...
            <h1 class="text-3xl font-light tracking-[0.2em] text-white uppercase italic">Access <span class="text-cyan-500 font-bold">Recovery</span></h1>
            <p class="text-slate-500 mt-4 text-[10px] font-bold uppercase tracking-[0.3em] px-8 leading-relaxed">
                Enter your identity link below to broadcast a recovery packet to your coordinates.
            </p>
        </div>
        ...
        <div class="bg-slate-950/40 backdrop-blur-2xl p-8 sm:p-10 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden">
            ...
            @if (session('status'))
                <div class="mb-8 p-4 rounded-lg bg-cyan-500/10 border-l-2 border-cyan-500 flex items-center gap-3">
                    <span class="text-[10px] font-bold text-cyan-400 uppercase tracking-widest leading-tight">
                        {{ session('status') }}
                    </span>
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-10">
                ...
                <div class="relative">
                    ...
                    @error('email')
                        <span class="text-[9px] text-rose-500 uppercase tracking-widest mt-2 block italic">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex flex-col gap-6">
                    <button type="submit" class="group relative w-full py-4 bg-white text-black font-bold text-[10px] uppercase tracking-[0.4em] rounded transition-all duration-300 hover:bg-cyan-400 hover:shadow-[0_0_25px_rgba(34,211,238,0.4)] active:scale-95 overflow-hidden">
                        <span class="relative z-10 italic">Transmit Recovery Link</span>
                    ...
                    <div class="text-center">
                        <a href="{{ route('login') }}" class="text-[9px] font-bold uppercase tracking-widest text-slate-500 hover:text-white transition-colors border-b border-white/5 hover:border-white pb-1">
                            [ Back to Terminal ]
                        </a>
                    </div>
                </div>
        ...
        <div class="mt-10 flex justify-center items-center gap-6">
            <div class="flex items-center gap-2">
                <span class="w-1.5 h-1.5 rounded-full bg-green-500/50 shadow-[0_0_8px_#22c55e]"></span>
                <span class="text-[8px] font-bold text-slate-600 tracking-[0.3em] uppercase">Broadcast: Active</span>
            </div>
            <div class="w-[1px] h-3 bg-white/10"></div>
            <span class="text-[8px] font-bold text-slate-600 tracking-[0.3em] uppercase italic">Node 0x1A22</span>
        </div>
    </div>
